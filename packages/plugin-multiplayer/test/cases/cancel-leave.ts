...
export async function testCancelLeave(): Promise<void> {
  ...
  await expect(
    ...
    "guest saw host_leaving",
  )

  const cancelRes = await hooks.tool.mp_cancel_leave.execute({}, makeToolContext())
  await expect(
    cancelRes.includes("cancelled") ||
      cancelRes.includes("Cancel") ||
      cancelRes.includes("aborted") ||
      cancelRes.includes("staying") ||
      cancelRes.includes("Leave cancelled"),
    "cancel ok",
  )

  await sleep(50)
  await expect(
    g1.messages.some((m) => m.type === "leave_cancelled"),
    "guest saw leave_cancelled",
  )

  const status = await hooks.tool.mp_status.execute({}, makeToolContext())
  await expect(
    status.includes("role: host") && !status.includes("leaving: pending"),
